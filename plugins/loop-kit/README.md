...
| モード | Claude Code | Codex | 使うスクリプト |
|---|---|---|---|
| session(Stop hook) | ✅ | ❌ | — (hook 方式。Codex は headless で hook が発火しない) |
| shell | ✅ | ✅ | Claude: `run-loop.sh` / Codex: `run-loop.codex.sh`(`codex exec`) |
| fanout | ✅ | ✅ | Claude: `fanout.sh` / Codex: `fanout.codex.sh`(`codex exec`) |
| skills(design/run/review) | ✅ | ✅ | SKILL.md を共有 |

Codex では session モードは使えないが、その用途は **shell モードで同じことができる**
(外側ループが hook の代わりに駆動する)ので機能的な穴はない。`.codex-plugin/plugin.json`
を持ち、skills / templates は Claude 版と共有する(他プラグインと同じ方式)。
