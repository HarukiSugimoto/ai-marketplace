#!/usr/bin/env bash
# promote.sh — 開発リポジトリの場所を解決し、資産の昇格と git 操作を補助する。
# キャッシュ(実行時コピー)ではなく必ず開発リポジトリ(原本)を操作する。
#
# 使い方:
#   promote.sh init [repoパス]           # このマシン用に config を作成/更新(新PCで最初に一度)
#   promote.sh path                      # 開発リポジトリの work-slides ディレクトリを表示
#   promote.sh asset <画像パス> [新名]    # 画像を repo の assets/ にコピー
#   promote.sh company <会社id>          # repo に会社パックの雛形を作成(_example を複製)
#   promote.sh commit "<メッセージ>"      # repo で add + commit(autoPush 設定なら push も)
set -euo pipefail

CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/work-slides/config.json"
...
if [ "${1:-}" = "init" ]; then
  if [ -n "${2:-}" ]; then
    ...
    if [ -z "$ROOT" ]; then
      ...
      echo "  bash <repo>/plugins/work-slides/scripts/promote.sh init <repoの絶対パス>" >&2
      exit 1
    fi
  fi
  ...
  [ "$PDIR" = "$PLUGIN_ABS" ] && PDIR="plugins/work-slides"   # 相対化に失敗したら既定
  # 既存 autoPush を保持
  AP="false"
...
if [ ! -f "$CONFIG" ]; then
  ...
  echo '  bash <repo>/plugins/work-slides/scripts/promote.sh init' >&2
...
case "$cmd" in
  ...
  asset)
    ...
    ;;
  company)
    ID="${2:?usage: promote.sh company <会社id>}"
    DEST="$PLUGIN/companies/$ID"
    if [ -d "$DEST" ]; then echo "already exists: $DEST" >&2; exit 1; fi
    cp -R "$PLUGIN/companies/_example" "$DEST"
    echo "created: $DEST"
    echo "次: $DEST/tokens.json(配色・書体)と rules.yaml(体裁規定)を会社に合わせて編集。"
    echo "    職場ディレクトリの .work-slides/workspace.yaml に company: $ID を書けば有効化。"
    ;;
  commit)
    ...
    if [ "$AUTOPUSH" = "true" ]; then
      ...
      echo "次: cd $REPO && git push   その後 各マシンで /plugin update work-slides"
    fi
    ;;
  *)
