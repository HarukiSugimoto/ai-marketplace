...
2. **構成案(outline)** ← 関門1。ストーリーの順番・主張をここで直す。想定反論への回答が
   どのスライドにあるかも表で示される
3. **HTML プレビュー** ← 関門2。同時に work-slide-validator が文字あふれ・スキーマ違反・
   会社規定(rules.yaml)違反・結論ファースト違反を機械チェック
4. 承認 → **編集可能 pptx** を案件の `slides/` に出力

文書タイプは4種(テンプレ = `templates/*.yaml`。yaml 1枚追加でタイプを増やせる):

| タイプ | 用途 | 特徴 |
|---|---|---|
| proposal | 提案書・企画書 | 想定反論つぶし・exec-summary 必須・next-action で締め |
| report | 報告書・進捗報告 | 「順調/遅延」を先頭で宣言。課題は対策つきで |
| regular | 定例会議資料 | 前回の next-action を自動で recap に転記(2回目以降) |
| minutes | 議事録ベース資料 | 会議メモ → 決定事項ファーストの清書資料 |

### その他の skill

| skill | 用途 |
|---|---|
| `/work-slide-build` | 既存 deck.json の差分修正・再ビルド(「3枚目直して」「pptxにして」) |
| `/company-add` | 新しい会社パックの追加(転職時)。テンプレ pptx があれば自動取り込み |
| `/catalog-add` | 気に入ったレイアウト・共通画像の資産化 |
...
`/company-add` が一連の手順を案内する。手動なら:

```bash
scripts/promote.sh company <会社id>                       # _example を複製して雛形を作る
node scripts/ingest-template.js <会社テンプレ.pptx> <会社id>  # 指定pptxから配色・書体を自動抽出
```

ingest は pptx 内の Office テーマ(theme1.xml)から accent / 書体を吸い出して
パックの tokens.json を書く。テーマ色が形骸化した pptx もあるため、**取り込み後は
必ずデモをビルドして目視確認**する。
