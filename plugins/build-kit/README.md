...
### 受入条件・タスク・ファイルは1本の鎖でつながっている

```
受入条件 ──→ 担当タスク ──→ 触ったファイル ──→ 判定するテスト
  plan.md      plan.md         changes.md          plan.md
               tasks/*.md                          テスト実行の出力
```

**この鎖は両端から引ける。** 「受入条件2 が危ない」→ タスク02 → `Reset.tsx`。
検証で差し戻しが出たとき、どこに戻ればいいかを `git diff` を読み直さずに引けます。

つなぎ目は2つあり、それぞれ守りが入っています:

| つなぎ目 | どこで守るか | 切れると何が起きるか |
|---|---|---|
| 受入条件 → 担当タスク | build-plan の分解後(空欄チェック)+ **ゲート2の確認項目3** | 誰も実装しない受入条件が、**検証まで進んでから** ⚠️ として現れる |
| タスク → ファイル | `changes.md` のタスク見出し → レポートの `task` 列 | 差し戻し先が引けず、`git diff` を頭から読み直すことになる |

**受入条件の側から数える。** タスクの側から見ると全部それらしく見えるので、
「受入条件が1つ余っている」ことはタスクを眺めても分かりません。

逆向き(受入条件に紐づかないタスク)は正常です。**prefactoring がそれに当たります** —
振る舞いを変えないので受入条件を持ちません。確認するのは受入条件 → タスクの片方向だけ。
...
| `build-plan` | 計画 | 受入条件を**テストの形**に落とし、**担当タスクを割り当て**、触るファイルを宣言し、全レイヤーを貫く縦割りのタスクに割って1タスク1ファイルで書き出す | `plan.md` + `tasks/` |
...
├── plan.md            # 全体制約、スコープ宣言、受入条件→担当タスク→テスト対応、依存の見取り図
├── tasks/
...
| **変更内容** | 変更ファイル・**どのタスクで触ったか**・理由の表 + **行番号つき差分** | `changes.md` + `git diff` |
...
| **受入条件 → 担当タスクの割り当て / 引けないものは単独タスクに落とす** | ナレッジグラフの entity resolution の fallback(未マッチ名を単独クラスタにする。しないと**alias マップに載らず静かに消える**)を、受入条件に転用 |
...
| **変更ファイルにタスク番号を持たせる**(`changes[].task`) | ナレッジグラフの provenance(全エッジに「どのワーカーが・どの文書から」を持たせ、統合側が元文書を読み直さずに済むようにする)の転用 |
